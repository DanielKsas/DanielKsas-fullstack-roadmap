...
function getBooksByChaptersNumber(numberChaptersMin) {

    let newArrayx2 = []

    for (const boooooooook of bookCollection) {

        if (boooooooook.chapters >= numberChaptersMin) {
            newArrayx2.push(boooooooook)
        }
...
// Función que reciba un número de páginas, y que te retorne un array con los TITULOS
// de los libros que tengan menos páginas que el parámetro

function filterByBookPages(numberPagesBook){
let ArrayNumbersTitles = []

for (const booooooooooooook of bookCollection) {
    
    if(booooooooooooook.pages <= numberPagesBook){

        ArrayNumbersTitles.push(booooooooooooook)
...
return ArrayNumbersTitles;

}

console.log(filterByBookPages(100))